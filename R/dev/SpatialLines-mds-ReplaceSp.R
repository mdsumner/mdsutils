...
linewkt <- function(x) {
    index <- 1:length(x)
    multi <- rep(FALSE, length(x))
    multi[grep("MULTILINESTRING", x)] <- TRUE


    x1 <- gsub("[(LINESTRING)]", "", d$WKT[!multi])
    x1 <- gsub("^\\s+", "", x1)

    x1.crds.list.txt <- lapply(strsplit(x1, ","), function(x) do.call("rbind", strsplit(x, " ")))
    x1.crds.list <- lapply(x1.crds.list.txt, function(x) matrix(as.numeric(x), nrow = nrow(x)))

    x2 <- gsub("[MULTILINESTRING)", "", d$WKT[multi])
    x2 <- gsub("^\\s+", "", x2)


    Lines.list <- vector("list", length(x1.crds.list))
    for (i in 1:length(Lines.list)) {
        if (multi[i]) {

        } else {
            x1.Lines.list[[i]] <- Lines(list(Line(x1.crds.list[[i]])), as.character(i))
    }

xx <-     SpatialLines(x1.Lines.list)
...
## why this warning? bbox handling in spplot.polygons
spplot(l3.dat)
#Warning message:
...
## rough grep for "Lines" "lines" for later testing in all dependent packages
